# SCADA Model Training - High Performance Architecture
# Uses your proven model architecture for SCADA data only
...
# Add parent directory to path so we can import from modules in parent directory
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
# Now import our custom modules
from features import SCADAFeatureExtractor
from scada_data_generator import SCADADataGenerator  # Our new data generator

# Your high-performance FocalLoss (same as your current model)
...
# Your high-performance model architecture (same as current)
def build_advanced_model(input_dim, num_faults, class_weights=None):
    """Build your high-performance model architecture for SCADA data"""
    ...
    # Enhanced fault classification branch
    ...
    # Severity and criticality branches
    ...
    model = tf.keras.Model(
        ...
        name="scada_transformer_model"
    )
    
    # Your high-performance optimizer setup
...
def train_scada_model():
    """Train SCADA model using your high-performance pipeline"""
    
    print("=" * 60)
    print("🔧 SCADA MODEL TRAINING - HIGH PERFORMANCE")
    print("=" * 60)
    
    # Load/generate SCADA dataset with real data reference
    print("📊 Generating SCADA dataset with real data patterns...")
    scada_gen = SCADADataGenerator()
    df = scada_gen.generate_realistic_dataset(n_samples=25000)
    
    print(f"✅ Generated {len(df)} SCADA samples")
    print(f"   Fault distribution: {df['fault_type'].value_counts().to_dict()}")
    
    # Feature extraction (your existing approach)
    print("🔄 Extracting SCADA features...")
    ...
    # Compute class weights (your existing approach)
    ...
    print(f"   Fault classes: {le.classes_}")
    
    # Build model using your architecture
    ...
    print("🚀 Starting training with class weights applied to FocalLoss...")
    
    # Train model (your existing approach)
    history = model.fit(
        X,
    ...
    try:
        model.save("scada_advanced_model.h5")
        print("✅ Successfully saved SCADA model")
    except Exception as e:
        print(f"❌ Error saving model: {e}")
        try:
            model.save("scada_advanced_model", save_format="tf")
            print("✅ Saved SCADA model in TensorFlow format")
        except Exception as e2:
            print(f"❌ Error saving in TF format: {e2}")
    
    # Save preprocessing artifacts
    joblib.dump(scaler, 'scada_scaler.pkl')
    joblib.dump(le, 'scada_fault_encoder.pkl')
    
    print("✅ Saved preprocessing artifacts:")
    print("   - scada_scaler.pkl")
    print("   - scada_fault_encoder.pkl")
    
    # Evaluation with TTA (your existing approach)
    ...
    # Load best model
    try:
        model = tf.keras.models.load_model('best_scada_model.h5', custom_objects={'FocalLoss': FocalLoss})
        print("✅ Successfully loaded best model with FocalLoss")
    except Exception as e:
    ...
    # Detailed evaluation
    print("\n=== SCADA MODEL EVALUATION RESULTS ===")
    ...
    print("\n🎉 SCADA model training complete!")
    print("📁 Model files:")
    print("   - best_scada_model.h5 or scada_advanced_model.h5")
    print("   - scada_scaler.pkl")
    print("   - scada_fault_encoder.pkl")
